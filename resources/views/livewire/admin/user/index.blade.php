<section id="userIndex" wire:key="userIndex">
    <x-slot name="title">
        User Management
    </x-slot>
    <x-slot name="button">
        @can('user-create')
            <a href="{{ route('admin.user.create') }}">
                <x-button-lg-right type="button">

                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                         stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                         class="h-6 w-6 mr-2">
                        <path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path>
                        <circle cx="8.5" cy="7" r="4"></circle>
                        <line x1="20" y1="8" x2="20" y2="14"></line>
                        <line x1="23" y1="11" x2="17" y2="11"></line>
                    </svg>

                    {!! "Add New User" !!}
                </x-button-lg-right>
            </a>
        @endcan
    </x-slot>
    <x-admin.success-msg/>
    <x-jet-validation-errors/>

    <div class="flex flex-col mt-2">

        <div class="py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
            <div class="flex flex-row justify-between items-center mb-2 ">
                <x-input class="w-2/4 md:w-1/4" name="search" wire:model.delay="search"
                         placeholder="Search user Name, Email or Role"></x-input>
                <label for="perPage"
                       class="flex flex-row items-center justify-end gap-2 text-gray-700 dark:text-gray-400">
                    <x-select class="w-1/2 " wire:model="perPage" id="perPage">
                        <option class="!text-gray-700" disabled>Select Per Page</option>
                        <option value="5">5</option>
                        <option value="10">10</option>
                        <option value="20">20</option>
                    </x-select>
                </label>
            </div>

            <div
                class="align-middle inline-block min-w-full shadow overflow-hidden sm:rounded-lg border-b border-gray-200">

            <table class="min-w-full" wire:loading.class="opacity-50">
                    <thead>
                    <tr>
                        <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 dark:bg-gray-700 text-left text-xs leading-4 font-medium text-gray-500 dark:text-gray-200 uppercase tracking-wider">
                            Name
                        </th>
                        <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 dark:bg-gray-700 text-left text-xs leading-4 font-medium text-gray-500 dark:text-gray-200 uppercase tracking-wider">
                            Role
                        </th>
                        <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 dark:bg-gray-700 text-left text-xs leading-4 font-medium text-gray-500 dark:text-gray-200 uppercase tracking-wider">
                            Register at
                        </th>
                        @if( Auth::user()->can('feature-user-set'))
                            <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 dark:bg-gray-700 text-left text-xs leading-4 font-medium text-gray-500 dark:text-gray-200 uppercase tracking-wider">
                                Featured
                            </th>
                        @endif
                        <th class="px-6 py-3 border-b border-gray-200 bg-gray-50 dark:bg-gray-700 text-left text-xs leading-4 font-medium text-gray-500 dark:text-gray-200 uppercase tracking-wider">
                            Actions
                        </th>
                    </tr>
                    </thead>
                    <tbody class="bg-gray-100 dark:bg-gray-800">
                    @forelse($users as $user)
                        <tr>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                <div class="flex flex-col justify-center items-start">
                                    <div
                                        class="flex flex-row text-sm leading-5 font-medium text-gray-900 dark:text-gray-200">
                                        {{ $user->name }}
                                        @if($user->email_verified_at )
                                            <div class="text-blue-500 ml-2 items-start justify-center">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                     stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                          d="M9 12.75L11.25 15 15 9.75M21 12c0 1.268-.63 2.39-1.593 3.068a3.745 3.745 0 01-1.043 3.296 3.745 3.745 0 01-3.296 1.043A3.745 3.745 0 0112 21c-1.268 0-2.39-.63-3.068-1.593a3.746 3.746 0 01-3.296-1.043 3.745 3.745 0 01-1.043-3.296A3.745 3.745 0 013 12c0-1.268.63-2.39 1.593-3.068a3.745 3.745 0 011.043-3.296 3.746 3.746 0 013.296-1.043A3.746 3.746 0 0112 3c1.268 0 2.39.63 3.068 1.593a3.746 3.746 0 013.296 1.043 3.746 3.746 0 011.043 3.296A3.745 3.745 0 0121 12z"/>
                                                </svg>
                                            </div>

                                        @endif
                                    </div>
                                    <div class="text-sm leading-5 text-gray-500">{{ $user->email }}</div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 text-gray-500">
                                @if($user->designation)
                                    <span class="flex flex-row items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                         stroke-width="1.5" stroke="currentColor" class="w-5 h-5 mr-2">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                              d="M20.25 14.15v4.25c0 1.094-.787 2.036-1.872 2.18-2.087.277-4.216.42-6.378.42s-4.291-.143-6.378-.42c-1.085-.144-1.872-1.086-1.872-2.18v-4.25m16.5 0a2.18 2.18 0 00.75-1.661V8.706c0-1.081-.768-2.015-1.837-2.175a48.114 48.114 0 00-3.413-.387m4.5 8.006c-.194.165-.42.295-.673.38A23.978 23.978 0 0112 15.75c-2.648 0-5.195-.429-7.577-1.22a2.016 2.016 0 01-.673-.38m0 0A2.18 2.18 0 013 12.489V8.706c0-1.081.768-2.015 1.837-2.175a48.111 48.111 0 013.413-.387m7.5 0V5.25A2.25 2.25 0 0013.5 3h-3a2.25 2.25 0 00-2.25 2.25v.894m7.5 0a48.667 48.667 0 00-7.5 0M12 12.75h.008v.008H12v-.008z"/>
                                    </svg>
                                    {{ $user->designation }}
                                         <hr class="border-1 border-blue-500/50 "/>
                                </span>
                                @endif
                                @foreach($user->roles as $role)
                                    <span class="px-1">
                                            {{ $role->name }}
                                        </span>
                                @endforeach

                            </td>

                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 font-medium">
                                <span class="text-indigo-600 hover:text-indigo-900">
                                    {{ $user->created_at }}
                                </span>
                            </td>


                            @if( Auth::user()->can('feature-user-set'))
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5">
                                    @if($user->profile)
                                        <label class="flex items-center relative w-max cursor-pointer select-none">
                                            <input type="checkbox" class="appearance-none transition-colors cursor-pointer
                                            w-14 h-7 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2
                                            focus:ring-offset-black focus:ring-blue-500 bg-red-500"
                                                   {{ $user->profile->feature== 1 ? 'checked' : '' }}
                                                   wire:click="setFeatured('{{ $user->id }}')"
                                            />
                                            <span
                                                class="absolute font-medium text-xs uppercase right-1 text-white"> OFF </span>
                                            <span
                                                class="absolute font-medium text-xs uppercase right-8 text-white"> ON </span>
                                            <span
                                                class="w-7 h-7 right-7 absolute rounded-full transform transition-transform bg-gray-200"/>
                                        </label>

                                    @endif


                                </td>
                            @endif


                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5">
                                <a href="#">
                                    <x-admin.show-button/>
                                </a>

                                @if(Auth::user()->can('user-edit') || Auth::user()->id == $user->id)
                                    @if(Auth::user()->id == $user->id)
                                        <a href="{{ route('admin.user.profile') }}">
                                            <x-admin.edit-button>Edit Profile</x-admin.edit-button>
                                        </a>
                                    @else
                                        <a href="{{ route('admin.user.edit', $user->id) }}">
                                            <x-admin.edit-button/>
                                        </a>
                                    @endif
                                @endif

                            </td>

                        </tr>
                    @empty
                        <tr>
                            <td colspan="{{ Auth::user()->can('feature-user-set')? 5:4 }}"
                                class="px-6 py-4 text-center text-gray-700 dark:text-gray-400 font-medium">
                               <span class="flex flex-row justify-center items-center gap-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                         stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                          d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z"/>
                                </svg>
                                   {!! 'No User Found' !!}
                               </span>
                            </td>
                        </tr>
                    @endforelse
                    </tbody>


                </table>
            </div>

        </div>
        {{ $users->links('pagination::tailwind') }}
    </div>


</section>



